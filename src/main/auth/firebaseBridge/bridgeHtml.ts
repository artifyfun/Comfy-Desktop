...
export function renderDoneHtml(): string {
  return shell(
    "You're signed in — Artify",
    `<div class="status-icon success">${CHECK_ICON}</div>
    <h1>You're signed in</h1>
    <p id="countdown">Returning to Artify in 3…</p>
    ...
    <script>
      (function(){
        ...
        function tick(){
          n -= 1;
          if (n <= 0) {
            el.textContent = 'Returning to Artify…';
          ...
          el.textContent = 'Returning to Artify in ' + n + '…';
 ...
 * The user can retry by returning to Artify and clicking
 * Sign in again.
 */
export function renderErrorHtml(message: string): string {
  return shell(
    'Sign-in failed — Artify',
    `<div class="status-icon error">${ERROR_ICON}</div>
    <h1>Sign-in failed</h1>
    <p>We weren't able to complete sign-in. Return to Artify and click Sign in again to retry.</p>
  ...
  <script type="module">
    ...
    function renderIdle(errorMessage) {
      ...
      contentEl.innerHTML = \`
        ...
        \${err}
        <div class="hint">You'll be returned to Artify automatically once sign-in completes.</div>
      \`
    ...
    try {
      ...
      contentEl.innerHTML = '<h1>Sign-in unavailable</h1><p>The Firebase SDK failed to load. Please reopen sign-in from Artify.</p>'
      throw err
    }
