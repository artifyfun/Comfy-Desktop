...
- **Quant guidance:** fp8 is safe from Ada (SM ≥ 8.9) upward — a UNETLoader on `weight_dtype: default` with an fp8 file should set `fp8_e4m3fn_fast`. NVFP4 targets Blackwell (SM ≥ 10.0) only; swap in an NVFP4 sibling when the GPU supports it. MXFP8 exists but is not recommended until loader support matures.
