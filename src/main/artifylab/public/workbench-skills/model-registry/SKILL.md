---
name: model-registry
description: Curated download URLs and target directories, organized by family (Flux, WAN, LTX, Qwen, Z-Image, SD15/SDXL), covering checkpoints, VAEs, text encoders, and LoRAs. Use when a workflow fails with a missing-model error, when the user asks where to get a model, or when setting up a new machine.
---

# Model Registry

One table per family: filename → source URL → target subdir under
`<COMFYUI>/models/`: each row gives the source URL and the target subfolder.
This registry grows with every release. If a model you need is missing, search
HuggingFace / CivitAI for the file and note the row here.
...
- CivitAI model-page URLs need resolving to the version's direct file link (`civitai.com/api/download/models/...`); early-access files require `CIVITAI_API_TOKEN`
...
Resolve the model page to its latest version's download link (which handles auth):
...
- Downloads resume on retry (HTTP Range), so re-run the same download after a
  network drop.
