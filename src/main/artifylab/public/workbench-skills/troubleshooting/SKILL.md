...
> To localize the failing stage, bisect the graph by previewing intermediate
> steps (run the workflow up to a single node, inspect its output). This guide
> is for runs that fail with an error, OOM, or missing node.

## Error Diagnosis Strategy

When a workflow fails, follow this approach:

1. Get the error. Read the execution result in ComfyUI's history (`GET /history/<prompt_id>`) or the server console — the full traceback plus any missing models/nodes is there
2. Check logs. Filter the server console output for `"error"`, `"warning"`, `"traceback"`
3. Identify the failing node. The history response includes the `node_id` and `node_type` that failed
4. Cross-reference inputs. Verify the failing node's expected input schema via the object-info API (`GET /object_info/<NodeType>`)
5. Check models. Verify all referenced model files exist in the matching `models/` subfolders
...
2. Use FP8/FP16 quantized models. FP8 Flux models use ~8GB vs ~24GB for FP16
   - Search for FP8 variants on HuggingFace / CivitAI (`flux fp8`, `sdxl fp8`)
...
3. Check the server console for import errors (filter for `import` / `error`).
...
1. Check output indices. Verify the exact output order via the object-info API (`GET /object_info/<NodeType>`)
...
1. Verify the model file exists in the matching `models/` subfolder
2. Check the exact filename. Model names in workflows must match the filename exactly (case-sensitive)
3. Re-download. If hash mismatch or corruption, fetch the file again from its source URL
...
1. Check current versions. The startup banner / `/system_stats` shows the
   PyTorch and CUDA versions ComfyUI is running with.
...
- Wrong port. The HTTP API defaults to 8188; ComfyUI Desktop uses 8000
...
```
GET /history                # All recent executions
GET /history/<prompt_id>    # One specific execution (full detail)
```
...
```
GET /system_stats   # GPU info, VRAM, Python/PyTorch versions
GET /queue          # Running and pending jobs
```

Recent error lines: read the server console output (filter for `error`).

### Verify Node Availability

```
GET /object_info                 # All installed node classes + schemas
GET /object_info/KSampler        # One node type's input/output schema
```

(A missing type here means the custom node pack is not installed/loaded.)

### Verify Models

Check the matching subfolders on disk:

```
<COMFYUI>/models/checkpoints/   # Installed checkpoints
<COMFYUI>/models/loras/         # Installed LoRAs
<COMFYUI>/models/controlnet/    # Installed ControlNets
```
...
| `Prompt outputs failed validation` | Node inputs don't match schema — check the node schema via `/object_info/<NodeType>` |
