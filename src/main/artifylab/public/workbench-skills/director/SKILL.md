...
- **Inspect node modes before each render.** After loading any workflow/template/subgraph and before running, check each node's mode on the canvas (bypassed/muted nodes show a badge). A `bypass` node is skipped (passes input through); a `mute` node and everything downstream don't execute. If the path, branch, or switch you need is bypassed or muted, set the wanted node `active` and the unwanted one `bypass`/`mute`. Never assume a switch or route is already active.
...
- **Confirm VIDEO renders via the filesystem, not /history.** For a `VHS_VideoCombine` / LTX / WAN clip, do NOT rely on ComfyUI's `/history` to confirm it exists. VHS-style video nodes write the .mp4 but frequently do NOT register an output in `/history` (prompt shows done, empty outputs, no error). Confirm the file in the output folder by `filename_prefix` + fresh mtime, then chain it forward as the next stage's input.
- **Bypass completed stages before queuing the next one.** If you build the multi-stage pipeline on ONE canvas (e.g. Krea2 → LTX → WAN) rather than running each phase in isolation, once a stage has run and its output is captured, bypass that stage's nodes BEFORE queueing the next stage. Otherwise the queue re-executes the whole graph and you pay for and wait on already-finished work, a real and costly failure mode. Keep only the active stage live; feed the prior output forward by registering the captured output as the consumer's input (see the staging note below).
...
2. register the output file as a ComfyUI input via the server API (`/view` → `/upload/image`); put the registered input filename into the consumer loader
...
To pipe ANY stage's output into the next stage's loader (`LoadImage` here, `VHS_LoadVideo` / `LoadAudio` in the video phases), register the output file as a ComfyUI input via the server API (`/view` → `/upload/image`) and drop the registered input filename into the loader's `image`/`video`/`audio` widget. For a file already on local disk, upload it the same way. NEVER copy the output file into, or guess, a filesystem `input/` path. ComfyUI's input and output directories may be CUSTOM (`--input-directory` / `--output-directory`), so a guessed path makes the loader reject the file (`Invalid image file`) and wastes the render.
