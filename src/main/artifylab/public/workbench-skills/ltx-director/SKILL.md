...
Set the `timeline_data` input on the node (find its node id in the graph) to the
JSON string value, in the graph editor or via the workflow JSON.
...
1. Put the file into ComfyUI's input dir via the server API (`/upload/image`), noting its `subfolder`/name
...
1. Find the `LTXDirector` node in the workflow graph and note its id/widgets.
...
5. Write the JSON back to the `timeline_data` input as a string.
6. Re-read to confirm it round-trips, then validate the workflow before queueing.
