...
> substitute a node you can't confirm is installed (check ComfyUI-Manager or the
> `/object_info` API).
...
1. Stage the example anywhere on disk (e.g. copy into the ComfyUI workflows
   folder).
2. Open it on the canvas (Workflow → Open, or drag the file in) — avoid pasting
   150KB of JSON by hand.
3. To SEE what is actually wired, resolve the graph yourself: collapse
   Get/Set/Reroute/bypass nodes to real links. This exposes both the dead text
   branch and the silently-reset dropdowns below. (Raw UI JSON hides them.)
...
that resolving the graph (Get/Set collapsed to real links) will show dangling.
...
sageattention…` and `triton: unavailable`) and with what ComfyUI-Manager has
installed.
...
- **Loading the example silently resets model/VAE/distill-LoRA dropdowns** to the
  ...
  load → strip → re-point" and re-point ALL of them. Resolve the graph (Get/Set
  collapsed to real links) to spot it.
...
are shared with `wan-t2v-video`; don't re-download. Install nodes via
ComfyUI-Manager.
