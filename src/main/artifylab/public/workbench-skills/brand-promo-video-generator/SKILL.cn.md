...
此适配版已移除第三方 Vibe Motion / Remotion 项目初始化与 npm 校验依赖，改为与生成面解耦的编排：来源研究、资产核验、故事规划、图像/视频生成、可选语音或音乐，以及后期合成。正常执行时不要初始化外部项目。
...
只有生成前硬确认通过后，才可以开始生成与剪辑。每次派发都应自包含：所选模型、画面比例、真实参考路径和用户原始需求。
