...
This adaptation replaces third-party Vibe Motion / Remotion implementation details with surface-agnostic orchestration: source research, asset verification, story planning, image/video generation, optional speech or music, and editing assembly. Do not initialize external projects or depend on npm validators during normal execution.

## Capability Coverage Rule

This Skill promises source lookup, image preparation/generation, video clip generation, optional speech/music/audio generation, and final editing/assembly. Before promising a final promo video, check what the current environment can actually do (image/video generation via MiniMax H3 — platform, web, or the local ComfyUI H3 workflow — plus an editor/ffmpeg for assembly). If generation or editing is not available here, downgrade the deliverable explicitly to a pre-production package instead of claiming that a final promo video can be generated.
...
## STEP 8: Produce assets

Start generation and editing only after the hard confirmation gate has passed. Keep each dispatch self-contained with the chosen model, aspect ratio, authentic reference paths, and original request.
