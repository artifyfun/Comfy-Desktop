...
> as a ComfyUI INPUT. When that frame is an OUTPUT from an earlier stage, register
> it as an input via the server API (`/view` → `/upload/image`) and drop the
> registered input filename into `LoadImage`.
> **NEVER copy the output file into, or guess, a filesystem `input/` path**. ComfyUI's
> input/output dirs may be CUSTOM (`--input-directory` / `--output-directory`), so a
> guessed path makes `LoadImage` reject the file (`Invalid image file`) and wastes
> the render. Going through the server API resolves the real dirs correctly.
...
> and no error. Do **NOT** conclude the render "silently dropped" from `/history`
> alone. Confirm the .mp4 exists in the output folder: match the `filename_prefix`
> (e.g. `ltxv2_….mp4`) and check the mtime is fresh, then register it as the next
> stage's input.
