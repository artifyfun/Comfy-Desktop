...
Use this Skill when the user wants to create, revise, audit, or generate music-video prompts or emotional short-film prompts where music, lyrics, typography, references, rhythm, performance, and camera language must be designed together. The workflow adapts MV prompt rules into a surface-agnostic production flow: key creative decisions are confirmed, locked prompts are delivered as final H3 prompts (written to a prompts file or handed over in chat), and media creation is planned as shot cards for the user's image/video/music pipeline.
...
## Surface-agnostic rules

- Do not assume third-party tools, shell scripts, browser plugins, or external generation APIs.
- Write locked prompts and revisions to a persistent prompts file (or deliver them in chat) rather than ephemeral tool state.
- Character cards, scene cards, typography cards, video clips, BGM, and final assembly are deliverables for the user's image/video/music pipeline (e.g. MiniMax H3 on the platform/web, or the local ComfyUI H3 workflow).
