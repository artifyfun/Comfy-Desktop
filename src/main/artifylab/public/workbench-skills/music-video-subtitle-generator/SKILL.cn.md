...
当用户需要创建、修改、审查或生成音乐视频提示词、情绪短片提示词时使用本 Skill，尤其是音乐、歌词、贴字、参考图、节奏、人物表演和镜头语言需要统一设计的任务。本 Skill 将第三方 MV prompt 规则适配为与生成面解耦的制作流程：关键创意决策需要确认，锁定 prompt 作为最终 H3 prompt 交付（写入 prompts 文件或直接给出），媒体生成规划为交给用户图像/视频/音乐管线的镜头卡。
...
- 锁定 prompt 和修订版本写入持久化的 prompts 文件（或直接在对话中交付），不依赖工具临时状态。
- 角色卡、场景卡、文字卡、视频片段、BGM 和最终合成是交给用户图像/视频/音乐管线的交付物（如 MiniMax H3 平台/网页，或本地 ComfyUI H3 工作流）。
- 不硬编码输出路径。
