...
> can't confirm is installed. Check ComfyUI-Manager or the `/object_info` API.
...
Check your `models/upscale_models/` folder first (common ones: `4x_foolhardy_Remacri`, the best
...
by that name. Install it via ComfyUI-Manager.
...
populates (ComfyUI caches model lists). Pull the file directly into
`models/frame_interpolation/`. Check what's already there (the core node's
dropdown / a folder listing) before installing anything.
