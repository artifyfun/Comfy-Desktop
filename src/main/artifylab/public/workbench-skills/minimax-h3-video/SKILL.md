...
These are core template-library graphs, **not** installer packs — the Template
Library is the canonical source.

**Load path:**

1. **Preferred.** Open **Template Library → Video → MiniMax H3: Text to
   Video** (or Image to Video / Reference to Video) in the ComfyUI frontend.
   Pick the local `video_minimax_h3_*` cards, **not** the `api_minimax_h3_*`
   paid partner templates.
2. **No template card available?** Fetch the UI JSON from
   ...
   into the ComfyUI workflows folder, then open it on the canvas.
...
All from `huggingface.co/Comfy-Org/MiniMax-H3` — download each file into the folder listed below.
...
2. Confirm the .mp4 exists in the output folder (fresh mtime).
   Video nodes often skip `/history`.
3. Register the last frame (or the whole clip) as the next stage's input via the
   server API (`/view` → `/upload/image`).
...
These graphs are not API-queue templates. Use the Template Library (or the
GitHub fetch path above) and download the weights from
