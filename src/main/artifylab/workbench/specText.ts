...
- 工具清单：wb_list_templates / wb_execute_template（wait=true 阻塞拿产物）/ wb_get_outputs（非阻塞查产物）/ wb_list_nodes（查节点图；无参=全量节点类型）/ wb_validate_workflow / wb_run_workflow / wb_clone_template / wb_publish_workflow / wb_remember / wb_forget / wb_build_workflow（一句话铺画布）/ wb_propose_plan（多步任务先出计划卡让用户拍板方向）。
