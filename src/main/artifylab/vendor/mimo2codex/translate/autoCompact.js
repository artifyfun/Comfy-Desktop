...
export function planChatCompaction(messages, opts) {
    if (messages.length < 4)
        return null;
    const total = estimateTokens(messages);
    if (total <= opts.atTokens)
        return null;
...
export function renderMessagesAsText(messages, maxChars = 200_000) {
    const lines = [];
    for (const m of messages) {
        let text = "";
        const c = m.content;
...
export const SUMMARY_SYSTEM_PROMPT = "You are compressing an in-progress coding-assistant conversation so it fits " +
...
export async function summarizeMiddle(middle, model, callChat, opts) {
...
const CACHE_MAX = 64;
export function summaryCacheKey(model, renderedMiddle) {
    return createHash("sha1").update(`${model}\n${renderedMiddle}`).digest("hex");
}
// For tests.
export function _clearSummaryCache() {
    summaryCache.clear();
}
...
export async function maybeCompactChat(chat, ctx) {
    ...
    if (summary === undefined) {
        ...
        try {
            ...
            log.warn(`auto-compact summary failed; leaving history intact: ${err.message}`);
            return { compacted: false };
        }
    ...
    const summaryMsg = {
        role: "user",
        content: `[Auto-compacted summary of ${plan.middle.length} earlier message(s), ` +
