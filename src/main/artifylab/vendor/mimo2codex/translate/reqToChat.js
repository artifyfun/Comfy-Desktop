...
function materializeStrippedImage(imageUrl, dropDir) {
    try {
        ...
        log.warn(`failed to materialize stripped image: ${e.message}`);
        return null;
    }
}
...
function partsToChatContent(parts, ctx) {
    ...
    if (droppedCount > 0) {
        log.warn(`dropped ${droppedCount} image part(s) — model "${ctx.model}" does not support image input (use mimo-v2.5 for vision); materialized ${droppedRefs.length} to disk`);
        ...
        out.push({
            type: "text",
            text: `[${droppedCount} image attachment${plural} omitted because the active model can't ingest images.\n` +
...
function toolOutputToString(output) {
    ...
    if (droppedImages > 0) {
        log.warn(`dropped ${droppedImages} image part(s) from tool output — Chat Completions tool messages cannot carry images`);
        chunks.push(`[${droppedImages} image attachment${droppedImages > 1 ? "s" : ""} omitted from tool output: this chat backend cannot ingest images in tool results.]`);
    }
    return chunks.join("");
}
...
function toolToChat(t, opts) {
    ...
    if (t.type === "tool_search") {
        ...
        if (typeof ts.description === "string")
            fn.description = ts.description;
        if (ts.parameters && typeof ts.parameters === "object")
            fn.parameters = ts.parameters;
        return { type: "function", function: fn };
    }
    ...
    warnOnce(t.type, `dropping unknown tool type "${t.type}" — harmless; if this should be supported, set MIMO2CODEX_VERBOSE=1 and report at https://github.com/7as0nch/mimo2codex/issues`);
...
function mcpToolToChat(t) {
    const label = t.server_label ?? t.connector_id ?? t.server_url ?? "(unnamed)";
    if (t.connector_id) {
        log.debug(`mcp first-party connector "${label}" dropped — advisory system note injected to upstream model. See doc/connector-plugins.md.`);
        return null;
    }
    if (t.server_url) {
        warnOnce(`mcp:remote-mcp:${label}`, `remote MCP tool "${label}" dropped — bridging not yet supported. See doc/connector-plugins.md.`);
        return null;
    }
    warnOnce(`mcp:unknown:${label}`, `mcp tool "${label}" dropped — neither connector_id nor server_url set.`);
    return null;
}
...
function collectFirstPartyConnectorLabels(tools) {
    const labels = [];
    for (const t of tools) {
        if (t.type !== "mcp")
            continue;
        const m = t;
        if (typeof m.connector_id !== "string" || m.connector_id.length === 0)
            continue;
...
function buildConnectorAdvisoryNote(labels) {
    ...
    const hintBlock = hintLines.length > 0
        ...
        : "";
    return (`Note from the mimo2codex proxy: the user has the following Codex Desktop connector plugin(s) ` +
...
const SECRET_KEY_RE = /^(authorization|api[_-]?key|token|secret|password|access[_-]?token|refresh[_-]?token|client[_-]?secret|bearer)$/i;
function redactToolPayload(t) {
    if (t === null || typeof t !== "object")
        return t;
    if (Array.isArray(t))
        return t.map(redactToolPayload);
    const out = {};
...
function dedupeToolsByName(tools) {
    ...
    const out = [];
    for (const t of tools) {
        const key = t.type === "function"
            ? `fn:${t.function.name}`
            : `builtin:${t.type}`;
        if (seen.has(key)) {
            const label = key.replace(/^(fn|builtin):/, "");
            log.warn(`dropping duplicate tool "${label}" — client sent it more than once. ` +
...
function sanitizeFunctionCallArguments(raw, name) {
    ...
    try {
        ...
        log.warn(`salvaging malformed historical tool_call arguments → "{}" — name="${name ?? "?"}" len=${raw.length} ` +
            `parse_err="${err.message}" preview=${JSON.stringify(raw.slice(0, 80))}`);
        return "{}";
    }
}
...
function removeOrphanToolMessages(messages) {
    let validIds = null;
    let i = 0;
        ...
        if (m.role === "assistant") {
            validIds =
                m.tool_calls && m.tool_calls.length > 0
                    ? new Set(m.tool_calls.map((tc) => tc.id).filter(Boolean))
                    : null;
            i++;
        }
        else if (m.role === "tool") {
            if (validIds && m.tool_call_id && validIds.has(m.tool_call_id)) {
                ...
                log.warn(`dropped orphan tool message: tool_call_id=${m.tool_call_id} (no preceding assistant.tool_calls in scope)`);
                messages.splice(i, 1);
...
function ensureToolCallsHaveOutputs(messages) {
...
export function reqToChat(req, opts = {}) {
    const messages = [];
    ...
    if (req.tools && req.tools.length > 0) {
        ...
        if (droppedCount > 0 && mapped.length === 0) {
            log.debug(`all ${req.tools.length} client tool(s) were filtered out — upstream will see no tools.`);
        }
        if (mapped.length > 0)
            chat.tools = dedupeToolsByName(mapped);
    ...
    if (opts.disableThinking !== true) {
        ...
        if (injected > 0) {
            log.info(`backfilled placeholder reasoning_content onto ${injected} historical assistant message(s) so thinking can stay ON for this request. ` +
