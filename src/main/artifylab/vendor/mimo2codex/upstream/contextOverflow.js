...
export function detectContextOverflow(input) {
    if (input.status !== 400)
        return null;
    const snippet = input.snippet;
    if (!snippet)
        return null;
    if (!POSITIVE_PATTERNS.some((re) => re.test(snippet)))
        return null;
    return {
...
export function detectMalformedJsonField(input) {
    if (input.status !== 400)
        return null;
    const snippet = input.snippet;
    if (!snippet)
        return null;
    if (!MALFORMED_JSON_FIELD_RE.test(snippet))
        return null;
    const lines = [];
    lines.push("Upstream rejected a malformed JSON field in the request (most likely a truncated tool_call.arguments).");
    lines.push("上游拒绝了请求里的一个畸形 JSON 字段（最大概率是被截断的 tool_call.arguments）。");
    lines.push("");
    lines.push("Why this can happen / 为什么会出现：");
    lines.push("  • The upstream cut a previous tool call mid-stream (length limit / network / cancel), ");
    ...
    lines.push("  • Upgrade to the latest mimo2codex — it sanitizes truncated tool calls automatically.");
    lines.push("    升级到最新版 mimo2codex（新版会自动清洗截断的 tool call）。");
    lines.push("  • If you can't upgrade right now: start a new codex session to drop the poisoned history.");
...
function buildFriendlyMessage(input) {
    const { modelId, contextWindow, snippet } = input;
    const lines = [];
    lines.push("Context length exceeded (上下文超出模型限制).");
    lines.push("The conversation history sent by codex exceeds the upstream model's context window.");
    if (modelId || contextWindow) {
        const parts = [];
        if (modelId)
            parts.push(`当前模型 ${modelId}`);
        if (contextWindow)
            parts.push(`上下文上限 ${contextWindow} tokens`);
        lines.push(`（${parts.join("，")}）`);
    }
    lines.push("");
    lines.push("How to recover / 如何恢复：");
    lines.push("  • In codex, run /compact to summarize and shrink the history, then retry.");
