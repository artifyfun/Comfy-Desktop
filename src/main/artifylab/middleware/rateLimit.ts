...
function cleanupExpiredRecords(): void {
  ...
  for (const ip in ipRequestCache) {
    const requests = ipRequestCache[ip]!
    const originalLength = requests.length
    ipRequestCache[ip] = requests.filter((timestamp) => timestamp > cleanupTime)
...
export function getRateLimitStats(): {
  ...
  const totalRequests = Object.values(ipRequestCache).reduce(
